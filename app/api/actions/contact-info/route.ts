import {
  ...
  getContactInfo,
  getAccount,
  getMostRecentTransactionHash,
} from "../transfer-sol/utils";
...
export const GET = async (req: Request) => {
  ...
  console.log(
    ...
    amount,
    contactInfo,
    mostRecentTransactionHash
  );

  // Fetch transactions that happened after the most recent transaction hash
  let latestTransaction = await fetchLatestTransactionAfterHash(
    toPubkey,
    mostRecentTransactionHash
  );
  ...
  if (
    latestTransaction &&
    latestTransaction.signature !== mostRecentTransactionHash &&
    contactInfo !== ""
  ) {
    ...
    const filePath = path.resolve(
      process.cwd(),
      "app/api/actions/contact-info/transaction.json"
    );
    ...
    const transactionExists = transactions.some(
      (tx: { transaction: string }) =>
        tx.transaction === latestTransaction.signature
    );
...
const fetchLatestTransactionAfterHash = async (
  toPubkey: PublicKey,
  ...
  const lastTransactionIndex = confirmedSignatures.findIndex(
    (signatureInfo) => signatureInfo.signature === lastTransactionHash
  );

  // Filter transactions that happened after the specified transaction hash
  const filteredTransactions =
    lastTransactionIndex !== -1
      ? confirmedSignatures.slice(0, lastTransactionIndex)
      : confirmedSignatures;
